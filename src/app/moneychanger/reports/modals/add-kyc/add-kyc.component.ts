...
export class AddKycComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,private mcReportService : MoneyChangerReportsService,
      private dialog : MatDialog, public dialogRef: MatDialogRef<AddKycComponent>,
  ...
  onSave(flag:string){
    ...
    if(flag == "ADD"){
     this.mcReportService.addKyc(this.buildAddPayload()).subscribe(data => {
       //success case
       this.loader = false ;
    ...
    (error:any)=>{ //error handling
      ...
      this.mcReportService.updateKyc(this.buildUpdatePayload()).subscribe(data => {
        //success case
        this.loader = false ;
        this.showSubmit = true ;
        this.dialogRef.close({data:data}) ;
