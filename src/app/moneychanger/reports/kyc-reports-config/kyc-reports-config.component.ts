...
@Component({
  selector: 'app-kyc-reports-config',
  templateUrl: './kyc-reports-config.component.html',
  styleUrls: ['./kyc-reports-config.component.scss','../../../../assets/styles/tables/table-style.scss'],

})
export class KycReportsConfigComponent implements OnInit {
   ...
   constructor(private titleService : TitleHeaderService, private mcReportService: MoneyChangerReportsService,
   ...
   searchKycService(){
    setTimeout(() => {
      this.loader = true ;
     this.mcReportService.searchKyc().subscribe((datas:any) => {
      //success case
       this.loader = false ;
   ...
   openAddKycDialog(name:string, flag:string, id:string, label:string, minVolume:string, maxVolume:string,
    minFrequency:string, maxFrequency:string
   ){

     this.dialog.open(AddKycComponent, {
       width: '615px',
       height: '365px',
