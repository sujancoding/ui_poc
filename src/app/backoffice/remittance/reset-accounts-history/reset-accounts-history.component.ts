...
@Component({
  selector: 'app-reset-accounts-history',
  templateUrl: './reset-accounts-history.component.html',
  styleUrls: ['./reset-accounts-history.component.scss', '../../../../assets/styles/tables/table-style.scss'],
  styles : [CommonSearchFilterCard]
})
export class ResetAccountsHistoryComponent implements OnInit {
 p: number = 1;
  ...
  ngOnInit(): void {
    this.titleService.setTitle('Reset Account History') ;
   ...
   setTimeout(()=>{
    this.loader = true ;
    this.agentSettlementService.getResetAccountHistory(this.selectedAccountNumber,this.dateGt,this.dateLt).subscribe(data =>{ 
      this.resetAccountHistory = data['accountDetails'];
      this.loader = false;
      console.log(this.resetAccountHistory);
    },
    (error:any) =>{
      this.loader = false;
      if(error.status != 401){
  ...
  searchFilter(){
    ...
    setTimeout(() => {
      ...
      this.agentSettlementService.getResetAccountHistory(this.selectedAccountNumber, this.dateGt , this.dateLt).subscribe((datas:any)=> {
        this.resetAccountHistory = datas['accountDetails'];
        console.log(this.resetAccountHistory)
      this.loader = false;
    },
  (error:any)=>{
